...
- Fixed Manage Skills folder-scoped disables so prompt filtering and explicit `/skill:<name>` blocking use the active folder context, including manager-owned plugin and custom-source skills that are omitted from discovery while disabled.

## [0.6.2] - 2026-06-29

### Fixed

- Kept the `/manage-skills` close and back shortcuts visible at narrow and common overlay widths so the skill manager no longer appears unclosable when footer hints are truncated.
...
[0.5.2]: https://github.com/leninkhaidem/pi-claude-plugin-manager/releases/tag/v0.5.2
